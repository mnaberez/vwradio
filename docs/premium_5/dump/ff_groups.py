...
'''
Find groups of consecutive locations.
'''
...
ffs = [int(x,16) for x in ffs]
groups = []
last_address = -2
current_range = []

for i, address in enumerate(ffs):
    if address == (last_address + 1):
        current_range.append(address)
    else:
        current_range = []
        current_range.append(address)
        groups.append(current_range)
    last_address = address

for r in groups:
    sys.stdout.write('%03d: ' % len(r))
    sys.stdout.write(', '.join(['0x%04x'%x for x in r]))
    sys.stdout.write('\n')
